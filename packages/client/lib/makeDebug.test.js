import { makeDebug, makeMatchFn } from "./makeDebug.js";
...
describe("makeMatchFn", () => {
  test("correctly parses filter string or regex", () => {
    const matchOne = makeMatchFn("*"); // Allow everything
    const matchTwo = makeMatchFn("test:*"); // Only allow things starting with 'test:'
    const matchThree = makeMatchFn("*,-test:*"); // Allow everything, but exclude channels starting with 'test:'
    const matchFour = makeMatchFn(/^test\:|name$/); // starts with 'test:' or ends with 'name'
    const matchFive = makeMatchFn("jwioefnm234,test:one");
