...
export class RepeatBlueprint {
  constructor(observable, renderFn, keyFn) {
  ...
  build({ appContext, elementContext }) {
    return new RepeatView(this.observable, this.renderFn, this.keyFn, appContext, elementContext);
  }
}
...
class RepeatView {
  ...
  constructor(observable, renderFn, keyFn, appContext, elementContext) {
    this.observable = observable;
    this.renderFn = renderFn;
