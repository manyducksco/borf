import { Pipe, PipeReceiverCallback } from "../__Pipe/Pipe";
import { Listenable } from "../__Source";
...
class FormStateErrors<T> implements Listenable<FormStateError<T>[]> {
  private pipe: Pipe<FormStateError<T>[]>;
  private listeners: Array<(errors: FormStateError<T>[]) => void> = [];
  current: FormStateError<T>[] = [];
  ...
  get(key: string) {
    return this.current.find((err) => err.key === key);
  }

  listen(callback: (errors: FormStateError<T>[]) => void) {
