...
form({
  children: [
    div({
      class: "errors",
      children: map(form.errors.subscribe(), "key", (error) =>
        li({
          children: [text(`${form.fieldNames[error.key]} ${error.message}`)],
        })
      ),
    }),
    input({
      type: "text",
      value: form.bind("firstName"),
      // TODO: have a value binding somehow also set element validity message when the value is invalid
      // The binding would have to include a way to subscribe to validation errors for its key (override 'bind' and 'subscribe')?
    }),
    input({
      type: "text",
      value: form.bind("lastName"),
    }),
    input({
      type: "number",
      value: form.bind("age"),
