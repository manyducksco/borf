  ...
  return {
    ...
    makeChannel(name) {
      ...
      return {
        ...
        log(...args) {
          if (options.log && match(this.name)) {
            for (const receiver of receivers) {
              receiver.receive(this.name, "log", ...args);
            }
          }
        },
        warn(...args) {
          if (options.warn && match(this.name)) {
            for (const receiver of receivers) {
              receiver.receive(this.name, "warn", ...args);
            }
          }
        },
        error(...args) {
          if (options.error && match(this.name)) {
            for (const receiver of receivers) {
              receiver.receive(this.name, "error", ...args);
            }
          }
        },
      };
