import { Bindable, Listenable, Stringifyable } from "../__Source";
import { isBinding, isBoolean, isListenable, isString } from "../__utils";
import { Component } from "../component/Component";
import { getClassMap } from "../component/utils/getClassMap";
import { toSameType } from "../component/utils/toSameType";
 ...
 * }]
 */
export type $Classes = $Class | $ClassMap | $ClassList;
type $Class = string | undefined | null | false;
type $ClassMap = {
  [className: string]: unknown | Listenable<unknown>;
};
type $ClassList = Array<$Class | $ClassMap | $ClassList>;
...
export class HTMLComponent extends Component {
  ...
  protected props: Readonly<$Attributes>;
  protected children: Component[] = [];
  protected cancellers: Array<() => void> = [];

  constructor(protected tagName: string, props?: $Attributes) {
    super();
  ...
  private applyAttrs() {
    for (const name in this.props) {
      ...
      if (!customProps.includes(name) && !eventRegex.test(name)) {
        const attr = this.props[name as keyof $Attributes];
