...
import { h, makeState, Readable, Writable, html } from "@borf/browser";
import { isFunction, isObject } from "@borf/bedrock";

function makeMockRouter() {
  return {
    $path: new Readable("/test"),
    $route: new Readable("/test"),
    $params: new Readable({}),
    $$query: new Writable({}),
...
function formatCollection(data) {
  ...
  for (const { fn, name } of fns) {
    ...
    const attributes = [];
    const decorators = [];
    const actions = { $$log: new Writable([]), fns: [] };
    let template;
...
catchLinks(root, (anchor) => {
  if (activeView) {
    ...
    activeView.actions.$$log.update((log) => {
      log.push({
        timestamp: new Date(),
