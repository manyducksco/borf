 ...
 */
export function assertFunction<T = (...args: unknown[]) => unknown>(
  value: unknown,
  errorMessage?: string
): value is T {
  if (isFunction(value)) {
...
export function isPromise<T = unknown>(value: unknown): value is Promise<T> {
  ...
  return (
    ...
    (isFunction(obj.then) && isFunction(obj.catch) && isFunction(obj.finally))
  );
}
 ...
 */
export function assertPromise<T = unknown>(
  value: unknown,
  errorMessage?: string
): value is Promise<T> {
  if (isPromise(value)) {
 ...
 */
export function extendsClass<T extends Function>(
  constructor: T,
  value: unknown
): value is Factory<T>;
 ...
 * @param value - A value that may extend `constructor`.
 */
export function assertExtends<T extends Function>(
  constructor: T,
  value: unknown,
  errorMessage?: string
): value is Factory<T>;
 ...
 * @param constructor - The constructor a value must be an instance of to match.
 */
export function assertInstanceOf<T extends Function>(
  constructor: T
): (value: unknown) => value is T;
 ...
 */
export function assertSet<T = unknown>(
  value: any,
  errorMessage?: string
): value is Set<T> {
  if (isSet(value)) {
...
export function assertNull(
  ...
  throw new TypeError(
    formatError(value, errorMessage ?? "Expected null. Got type: %t, value: %v")
  );
}
